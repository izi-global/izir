'''A basic example of authentication requests within a izi API'''
import izi
import jwt

# Several authenticators are included in izi/authentication.py. These functions
...
# request object.
@izi.get('/authenticated', requires=authentication)
def basic_auth_api_call(user: izi.directives.user):
...
api_key_authentication = izi.authentication.api_key(api_key_verify)


@izi.get('/key_authenticated', requires=api_key_authentication)  # noqa
def basic_auth_api_call(user: izi.directives.user):
...
token_key_authentication = izi.authentication.token(token_verify)


@izi.get('/token_authenticated', requires=token_key_authentication)  # noqa
def token_auth_call(user: izi.directives.user):
    return 'You are user: {0} with data {1}'.format(user['user'], user['data'])


@izi.post('/token_generation')  # noqa
