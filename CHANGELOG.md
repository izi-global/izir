...
- Implemented improved class based directives with cleanup support (see: https://github.com/izi-global/izir/pull/603)
- Support ujson if installed
- Implement issue #579: Allow silencing intro message when running izi from command line
- Implemented issue #531: Allow exit value to alter status code for CLI tools
- Updated documentation generation to use izi's JSON outputter for consistency
...
- Improved izi.test decode behaviour
- Added built in handlers for CORS support:
    - directive `izi.directives.cors`
...
@izi.get(inputs={'application/json': my_input_formatter})
def foo():
    pass
```
- Adds support for passing in a custom scheme in izi.test
- Adds str() and repr() support to izi_timer directive
...
- Added support for passing `params` dictionary and `query_string` arguments into izi.test.http command for more direct modification of test inputs
- Added support for manual specifying the scheme used in izi.test calls
...
- Breaking Changes
    ...
    - Fixed issue #432: Improved ease of sub classing simple types - causes type extensions of types that dont take to __init__
                        ...
                        after the fact
    - Fixed issue #405: cli and http @izi.startup() differs, not executed for cli, this also means that startup handlers
...
- Added support for getting URL from izi function
- Added support for using `izi.local()` on methods in addition to functions
- Added a default mime-type for static file endpoints (`application/octet-stream`)
- Added initial `izi.API(__name__).context` dictionary as a safe place to store global per-thread state (such as database connections)
...
- Improved how `izi.test` deals with non JSON content types
...
- Fixed a bug in the `izi_documentation` directive
- Breaking Changes
...
### 2.0.7 - Mar 25, 2016
- Added convenience `put_post` router to enable easier usage of the common `@izi.get('url/', ('PUT', 'POST"))` pattern
- When passing lists or tuples to the izi http testing methods, they will now correctly be handled as multiple values
...
### 2.0.3 - Mar 22, 2016
- Fixes izi.use module on Windows

### 2.0.2 - Mar 18, 2016
- Work-around bug that was keeping izi from working on Windows machines
- Introduced a delete method to the abstract izi store module

### 2.0.1 - Mar 18, 2016
- Add in-memory data / session store for testing
- Default izi.use.HTTP to communicate over JSON body

### 2.0.0 - Mar 17, 2016
- Adds the concept of chain-able routing decorators
- Adds built-in static file handling support via a `@izi.static` decorator (thanks @BrandonHoffman!)
- Adds a directive to enable directly accessing the user object from any API call (thanks @ianthetechie)
- Adds the concept of seamless micro-services via the izi.use module, enable switching between HTTP and local without code change
- Adds built-in support for 'X-Api-Key' header based authentication via `authentication.api_key`
- Adds support for running arbitrary python functions at runtime via a `@izi.startup` decorator
- Adds support for smarter handling of html output types
- Adds a UUID type
- Adds support for explicit API creation / referencing using `izi.api(__name__)`
- Adds a logging middleware to simplify the process of logging all requests with izi
...
- Added support for easily extending izis JSON outputter with support for custom types
...
- Added support for applying type annotations and directives locally with @izi.local()
- Added support for a base_url by doing `izi.API(__name__).http.base_url = '/base_url'`
- Added support for automatically running CLI commands from izi command line runner
...
- Improved integration with Falcon so that primary elements (like status codes) can be imported directly from izi
...
- Fully re-factored izi's type system for easier extensibility
- Breaking Changes
    - directives are no longer automatically applied to local function calls, '@izi.local' must be used to apply them
    ...
    - API singleton now has sub-apis for each interface in use (IE izi.API(__name__).http and izi.API(__name__).cli)
    - run module has been removed, with the functionality moved to izi.API(__name__).http.server() and the terminal functionality
      being moved to izi.development_runner.izi

### 1.9.9 - Dec 15, 2015
- IZIR's json serializer will now automatically convert decimal.Decimal objects during serializationkw
- Added `in_range`, `greater_than`, and `less_than` types to allow easily limiting values entered into an API

### 1.9.8 - Dec 1, 2015
- IZIR's json serializer will now automatically convert returned (non-list) iterables into json lists
...
### 1.9.3 - Nov 18, 2015
- Enabled `izi.types.multiple` to be exposed as nargs `*`
- Fixed a bug that caused a CLI argument when adding an argument starting with `help`
- Fixed a bug that caused CLI arguments that used `izi.types.multiple` to be parsed as nested lists

### 1.9.2 - Nov 18, 2015
- Improved boolean type behavior on CLIs

### 1.9.1 - Nov 14, 2015
- Fixes a bug that caused izi cli clients to occasionally incorrectly require additional arguments
...
### 1.8.2 - Nov 9, 2015
- Drastically improved izi performance when dealing with a large number of requests in wsgi mode

### 1.8.1 - Nov 5, 2015
- Added `json` as a built in izi type to handle urlencoded json data in a request
- Added `multi` as a built in izi type that will allow a single field to be one of multiple types
...
### 1.6.0 - Oct 13, 2015
- Adds support for attaching izi routes to method calls
- IZIR is now compiled using Cython (when it is available) for an additional performance boost
...
### 1.3.1 - Sep 8, 2015
- Fixed string only annotations causing exceptions when used in conjunction with `izi.cli`
- Fixed return of image file not correctly able to set stream len information / not correctly returning with PIL images
- Added examples of image loading with izi

### 1.3.0 - Sep 8, 2015
- Started keeping a log of all changes between releases
- Added support for quickly exposing functions as cli clients with `izi.cli` decorator
- Added support for quickly serving up development APIs from withing the module using: `if __name__ == '__main__': __izi__.serve()`
...
- Added support for returning streams within iziged functions
- `izi.types.decimal` renamed to `izi.types.float_number` and `izi.types.decimal` type added that returns python Decimal
- `izi.types.accept` wrapper added, that makes it easy to customize doc strings and error handling for any preexisting type converter
